...
function isValid(input) {
    if (input.value.trim() === "") {
        ...
        input.nextElementSibling.innerText = `Error! - Empty input`
        return false;
    }
...
function save() {
    ...
    const streetNumValid = isValid(streetNum);
    const postalCodeValid = isValid(postalCode);
    if (cityValid && streetValid && streetNumValid && postalCodeValid) {
        const myData = {
            city: city.value,
            street: street.value,
