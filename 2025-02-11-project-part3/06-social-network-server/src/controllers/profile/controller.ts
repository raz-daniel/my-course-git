...
export async function getProfile(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        if (!user) {
            return next(new TwitterError(StatusCodes.NOT_FOUND, 'User not found!'))
        }

        const plainUser = user.get({ plain: true })
        if (!plainUser.posts || plainUser.posts.length === 0) {
            return next(new TwitterError(StatusCodes.NOT_FOUND, 'No Posts found'))
        }
        ...
        next(new TwitterError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to retrieve profile posts'))
...
export async function getPost(req: Request<{ id: string }>, res: Response, next: NextFunction) {
    try {
        ...
        if (!req.params.id) {
            return next(new TwitterError(StatusCodes.BAD_REQUEST, 'cannot find post id'))
        }
        ...
        next(new TwitterError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to retrieve post'))
    }
}
...
export async function deletePost(req: Request<{ id: string }>, res: Response, next: NextFunction) {
    try {
        ...
        if (deletedCount === 0) {
            return next(new TwitterError(StatusCodes.NOT_FOUND, 'Posts not found'))
        }
        ...
        next(new TwitterError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to delete post'))
    }
}
...
export async function createPost(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        next(new TwitterError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to create post'))
    }
...
export async function updatePost(req: Request<{ id: string }>, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        if (!title || !body) {
            return next(new TwitterError(StatusCodes.BAD_REQUEST, 'Title and Body are Required'))
        }
        ...
        next(new TwitterError(StatusCodes.INTERNAL_SERVER_ERROR, 'Failed to update post'))
    }
}