const num = 123;
console.log(`typeof num is: ${typeof num}`);

const firstName = `beery`;
console.log(`typeof name is: ${typeof firstName}`);

const iAmHappy = true;
console.log(`typeof iAmHappy is: ${typeof iAmHappy}`);

const iAmHSad = 4<2;
console.log(`typeof iAmSad is: ${typeof iAmHappy}`);

const calc = 4+22;
console.log(`typeof calc is: ${typeof calc}`);

let dayOfWeek;
console.log(`typeof dayOfWeek is: ${dayOfWeek}`);

const itay = {name: `itay`, type: `person`, age: 25, average: 90.20}
console.log(`typeof itay is: ${typeof itay}`);

console.log(`typeof itay is: ${typeof itay.type}`);

console.log(`typeof itay is: ${typeof itay.birthdate}`);

console.log(`typeof itay is: ${typeof itay.average}`);

// alert(`Hello`)
console.log(`typeof alert is: ${typeof alert}`);

const animals = [`dog`, `cat`, `fish`];
console.log(`typeof animals is: ${typeof animals}`);

console.log(`itay age is represented as ${typeof itay.age === `number` ? `number` : `strubg`}`);

if (typeof itay.age !== `number`) {itay.age = +itay,age }

